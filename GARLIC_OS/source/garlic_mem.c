...
typedef struct
{ // Estructura para cada entrada de la tabla de segmentos
	Elf32_Word p_type;
	Elf32_Off p_offset;
	Elf32_Addr p_vaddr;
	Elf32_Addr p_paddr;
...
intFunc _gm_cargarPrograma(int zocalo, char *keyName)
{
	...
	// variables iniciales relacionadas con el cargar el vector en memoria dinï¿½mica
	long lSize;
	char *buffer;
	size_t result;
	...
	// obtener tamaï¿½o de la file
	fseek(pFile, 0, SEEK_END);
	lSize = ftell(pFile);
	fseek(pFile, 0, SEEK_SET);

	// dar tamaï¿½o a la memoria para que contenga todo el fichero
	...
	// direcciï¿½n enviada como result
	...
	for (i = 0; i < num_st; i++)
	{
		...
		if (segment_type == 1 && i == 0)
		{
			...
			// obtencion direcciï¿½n inicial del segmento a cargar y desplazamiento y size programa
			desp_prog = segments_table.p_offset;
			dir_ref = segments_table.p_paddr;
			size_prog = segments_table.p_memsz;

			// reserva memoria, devuelve la primera posiciï¿½n o
			...
			if (prim_pos != 0)
			{
				...
				// damos valor a la direcciï¿½n inicial de donde se encuentra el programa en memoria
	...
	return ((intFunc)dirprog); // devuelve la direcciï¿½n del programa en que se encuentra en el segmento
}
