/*------------------------------------------------------------------------------
	...
	(carga de programas con 2 segmentos, listado de programas, gesti�n de
	 franjas de memoria)
...
/* Funci�n para escoger una opci�n con un bot�n (tecla) de la NDS */
int leerTecla(int num_opciones)
{
	int i, j, k;

	i = -1;							// marca de no selecci�n
	do {
		_gp_WaitForVBlank();
		gestionSincronismos();
		scanKeys();
		...
		if (k != 0)
			for (j = 0; (j < num_opciones) && (i == -1); j++)
				if (k & bitsTeclas[j])
					i = j;			// detecci�n de una opci�n v�lida
	} while (i == -1);
	return i;
}


/* Funci�n para presentar una lista de opciones de tipo 'string' y escoger una */
...
/* Funci�n para presentar una lista de opciones de tipo 'n�mero' y escoger una */
	...
	{	while (1)
		{
													...
													|| (_gd_pcbs[3].PID == 0))
			{
				...
				j = 0;
				for (i = 1; i <= 3; i++)	// detectar z�calos disponibles
				{	if (_gd_pcbs[i].PID == 0)
